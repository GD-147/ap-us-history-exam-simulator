...
SRC_DIR = Path("imports/ap_us_history_exams/txt")
PDF_SRC_DIR = Path("imports/ap_us_history_exams/pdf")
OUT_DIR = Path("packs/ap-us-history/data")
PDF_OUT_DIR = Path("packs/ap-us-history/pdf")
CONFIG_PATH = Path("packs/ap-us-history/config.json")

ID_RE = re.compile(r"^(APUSH-(MCQ|SAQ|DBQ|LEQ)(\d+)-(\d{3}))$")
CHOICE_RE = re.compile(r"^([A-D])\)\s*(.*)$")

MCQ_KEY_RE = re.compile(
    r"^(APUSH-MCQ(\d+)-\d{3})\s+[—–-]\s+Correct:\s+([A-D])\s+[—–-]\s+Correct Answer:\s+(.*?)\s+[—–-]\s+Explanation:\s+(.*)$"
)
...
SECTION_FILE_PREFIX = {
    "mcq": "ap_us_history_mcq_exam",
    "saq": "ap_us_history_saq_exam",
    "dbq": "ap_us_history_dbq_exam",
    "leq": "ap_us_history_leq_exam",
}

SECTION_TITLE = {
    "mcq": "AP U.S. History MCQ Practice Exam",
    "saq": "AP U.S. History SAQ Practice Exam",
    "dbq": "AP U.S. History DBQ Practice Exam",
    "leq": "AP U.S. History LEQ Practice Exam",
}
...
def parse_mcq_key(key_text: str):
    ...
    for raw in key_text.splitlines():
        ...
        elif line.startswith("APUSH-MCQ"):
            ignored.append(line)
    return key, ignored
...
def expected_ids(kind: str, exam_no: int, count: int):
    return [f"APUSH-{kind}{exam_no}-{n:03d}" for n in range(1, count + 1)]
...
def update_printables():
    ...
    for pdf in sorted(PDF_OUT_DIR.glob("*.pdf")):
        ...
        if "mcq" in stem:
            label = f"AP U.S. History MCQ Practice Exam{exam_label}"
        elif "frq" in stem or "free_response" in stem or "free-response" in stem:
            label = f"AP U.S. History Free-Response Practice Exam{exam_label}"
        elif "saq" in stem:
            label = f"AP U.S. History SAQ Practice Exam{exam_label}"
        elif "dbq" in stem:
            label = f"AP U.S. History DBQ Practice Exam{exam_label}"
        elif "leq" in stem:
            label = f"AP U.S. History LEQ Practice Exam{exam_label}"
        else:
            label = f"AP U.S. History Practice Exam{exam_label}"
...
def process_file(path: Path):
    ...
    if not items:
        raise ValueError(f"{path.name}: no APUSH question IDs found.")

    written = []

    mcq_items = [x for x in items if x["id"].startswith("APUSH-MCQ")]
    if mcq_items:
        mcq_key, ignored = parse_mcq_key(key_text)
        if ignored:
            raise ValueError(f"{path.name}: malformed MCQ answer-key lines:\n" + "\n".join(ignored))

        exam_numbers = sorted({int(re.match(r"APUSH-MCQ(\d+)-", x["id"]).group(1)) for x in mcq_items})
    ...
    for kind in ["SAQ", "DBQ", "LEQ"]:
        section_items = [x for x in items if x["id"].startswith(f"APUSH-{kind}")]
        if not section_items:
            continue

        exam_numbers = sorted({int(re.match(rf"APUSH-{kind}(\d+)-", x["id"]).group(1)) for x in section_items})
        if len(exam_numbers) != 1:
